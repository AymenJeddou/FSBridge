...
function buildPdf(opts: {
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(18);
  doc.text(DOC_LABELS[opts.type].toUpperCase(), pageW / 2, 48, { align: "center" });
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(110);
  doc.text(opts.isApproved ? "Document officiel approuvé" : "Brouillon — Document non officiel", pageW / 2, 54, { align: "center" });

  if (!opts.isApproved) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    doc.setTextColor(200, 50, 50); // Red warning
    doc.text("BROUILLON EN ATTENTE D'APPROBATION ADMINISTRATIVE", pageW / 2, 62, { align: "center" });
    doc.setTextColor(10); // reset color
  }
  ...
  doc.setFontSize(7);
  doc.setFont("helvetica", "bold");
  doc.text(opts.isApproved ? "APPROUVÉ" : "BROUILLON", margin + 18, footerY - 2, { align: "center" });
  ...
  doc.line(pageW - margin - 75, footerY + 5, pageW - margin, footerY + 5);

  if (opts.isApproved) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    doc.text("Le Doyen de l'Université", pageW - margin, footerY - 5, { align: "right" });
    doc.setFont("courier", "bolditalic");
    doc.setFontSize(10);
    doc.setTextColor(30, 80, 180); // royal blue digital signature
    doc.text("Signé numériquement par le Doyen", pageW - margin, footerY + 2, { align: "right" });
    doc.setTextColor(10); // reset color
  } else {
    doc.setFont("helvetica", "italic");
    doc.setFontSize(8);
    doc.setTextColor(120);
    doc.text("En attente de signature du Doyen", pageW - margin, footerY + 2, { align: "right" });
    doc.setTextColor(10); // reset color
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { requestId, action = "generate" } = await req.json();
    if (!requestId) throw new Error("requestId manquant");



    // If admin action, verify role
    if (action === "approve" || action === "refuse") {
      ...
      if (!roleCheck) {
        throw new Error("Privilèges insuffisants pour cette action administrative.");
      }
    }
    ...
    if (action === "refuse") {
      ...
      return new Response(JSON.stringify({ ok: true, decision: "refuse" }), {
    ...
    if (action === "approve") {
      ...
      const pdfBytes = buildPdf({ type: docReq.type, profile, refNum, isApproved: true, ...extras });
    ...
    // Default "generate" (student draft) action
    // ===== Auto-approval AI logic using Gemini =====
    let decision = "approuve";
    let reason = "Étudiant inscrit, documents requis disponibles, demande conforme. En attente de signature.";
...
Motif : ${docReq.motif || "(aucun)"}`;
    ...
    const refNum = `EP-${new Date().getFullYear()}-${requestId.slice(0, 8).toUpperCase()}`;
    const pdfBytes = buildPdf({ type: docReq.type, profile, refNum, isApproved: false, ...extras });
    ...
    await admin.from("document_requests").update({
      statut: "en_attente", // Keeps it yellow!
      decision_ia: reason,
      pdf_url: signed?.signedUrl || path,
      reviewed_at: new Date().toISOString(),
    }).eq("id", requestId);

    return new Response(JSON.stringify({ ok: true, decision: "en_attente", url: signed?.signedUrl }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e: any) {
